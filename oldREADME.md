...
After each iteration, Loop updates the project's `CLAUDE.md` with learnings. This works because `loop.sh` passes `CLAUDE.md` to each Claude Code instance via the `-p` flag, so future iterations (and future human developers) benefit from discovered patterns, gotchas, and conventions.
